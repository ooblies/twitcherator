...
function bigMultiplyStr(number, multiplier) {
  num = getBigNumberFromStr(number);
  mult = getBigNumberFromStr(multiplier);

  return bigMultiply(num, mult);
}
...
function bigAddStr(number, increment) {
  number = number.replace(/,/g,"");
  increment = increment.replace(/,/g,"");
  
  num = getBigNumberFromStr(number);
  inc = getBigNumberFromStr(increment);

  return getBigNumberAsString(bigAdd(num,inc));
}
  ...
  while (j >= 0 || overflow > 0) {
    ...
    if (j >= 0) {
      ...
      if (newNum.toString().length > segmentLength) {
        ...
        var calc = (parseInt(newNum) + parseInt(overflow)).toString();
        if (calc == "0") {
          calc = "00000";
        }
        num[i] = calc;
      ...
      var calc = (parseInt(num[i]) + parseInt(overflow)).toString();
      if (calc == "0") {
        calc = "00000";
      }
      num[i] = calc;
...
function getOnePercent(number) {
  var num = number.replace(/,/g,"");

  var toAdd = num.substr(0, num.length - 2);
